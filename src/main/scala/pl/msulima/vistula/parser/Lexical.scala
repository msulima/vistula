...
object Lexical {
  ...
  val letter = P(P(CharIn('a' to 'z')) | P(CharIn('A' to 'Z')))
  val digit = P(CharIn('0' to '9'))
  val identifier: P[Ast.identifier] = {
    val firstLetter = letter | "_"
    val rest = (letter | digit | "_").rep

    P(firstLetter ~ rest).!.filter(!keywordList.contains(_)).map(Ast.identifier)
  }

  val escapeseq: P0 = P("\\" ~ AnyChar)

  val shortstring: P[String] = {
  ...
  val stringliteral: P[String] = {
    val longstring: P[String] = {
      ...
      P(longstring0("'''") | longstring0("\"\"\""))
    }

    P(longstring | shortstring)
  }
